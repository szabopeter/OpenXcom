 ...
 */
TransferItemsState::TransferItemsState(Game *game, Base *baseFrom, Base *baseTo) : State(game), _baseFrom(baseFrom), _baseTo(baseTo), _baseQty(), _transferQty(), _soldiers(), _crafts(), _items(), _sel(0), _total(0), _pQty(0), _cQty(0), _aQty(0), _iQty(0), _hasSci(0), _hasEng(0), _distance(0.0), _itemOffset(0)
{
	...
	_storageLimitsEnforced = Options::getBool("storageLimitsEnforced");
...
void TransferItemsState::increaseByValue(int change)
{
	...
	if (TRANSFER_CRAFT == selType )
	{
		...
		if (_storageLimitsEnforced && _baseTo->storesOverfull(_iQty + (int)(craft->getItems()->getTotalSize(_game->getRuleset()) * 10 + 0.5)))
		{
			_timerInc->stop();
	...
	if (TRANSFER_ITEM == selType && selItem->getAlien()
		&& _storageLimitsEnforced * _aQty + 1 > _baseTo->getAvailableContainment() - _storageLimitsEnforced * _baseTo->getUsedContainment())
	{
		_timerInc->stop();
	...
	{
		int freeContainment = _storageLimitsEnforced ? _baseTo->getAvailableContainment() - _baseTo->getUsedContainment() - _aQty : INT_MAX;
